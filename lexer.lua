...
local function Location(start, length)
	...
	return {
		...
		End = start+length,
		Length = length,
		Raw = code:sub(start, start+length)
	}
end
...
local function Lexeme(position, kind)
	...
	print(game.HttpService:JSONEncode(token), debug.traceback())
...
local function treat()
	...
	elseif currentChar == "(" 
		...
		consume()
		print(currentChar)
		return Lexeme(Location(start, 0), currentChar)
...
print(game.HttpService:JSONEncode(Tokens))
